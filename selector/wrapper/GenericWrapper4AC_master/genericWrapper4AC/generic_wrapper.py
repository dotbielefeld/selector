...
from wrapper.GenericWrapper4AC_master.genericWrapper4AC.argparser.parse import parse, get_parser, get_extended_parser
...
class AbstractWrapper(object):
    ...
    def set_tmpdir(self, tmp_dir):
        '''
            set temporary directory for logger files;
    ...
    def call_target(self, target_cmd: str):
        ...
        self._watcher_file = NamedTemporaryFile(
            suffix=".logger", prefix="watcher-%d-" % (random_id), dir=self.data.tmp_dir, delete=False)
        self._solver_file = NamedTemporaryFile(
            suffix=".logger", prefix="solver-%d-" % (random_id), dir=self.data.tmp_dir, delete=False)
